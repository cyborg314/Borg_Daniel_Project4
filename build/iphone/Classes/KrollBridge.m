...
NSString * Borg_Daniel_Project4$ModuleRequireFormat = @"(function(exports){"
...
@implementation Borg_Daniel_Project4Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_borg_daniel_project4);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Borg_Daniel_Project4 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Borg_Daniel_Project4" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_borg_daniel_project4 = [[Borg_Daniel_Project4Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_borg_daniel_project4];
	
	NSString *_borg_daniel_project4NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _borg_daniel_project4NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_borg_daniel_project4 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Borg_Daniel_Project4$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.borg_daniel_project4.kroll" 
